...
class Settings(BaseSettings):
    # "Защита от утечек": секреты только через .env (который НЕ коммитим) и переменные окружения.
    ...
    consult_url: str | None = Field(default=None, alias="CONSULT_URL")            # ссылка на запись на консультацию

    # LLM (через Ollama по HTTP)
    ...
    # Лимиты (по ТЗ)
    daily_sessions_limit: int = Field(default=3, alias="DAILY_SESSIONS_LIMIT")      # 2–3 сессии/день
    session_max_followups: int = Field(default=6, alias="SESSION_MAX_FOLLOWUPS")    # 5–6 уточнений/сессию
